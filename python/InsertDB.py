...
def insert_news(newsinsert, check):
    ...
    try:
        ...
        with conn.cursor() as curs:
            ...
            sql2 = 'UPDATE today_news set today= %s, href= %s, title= %s' \
                    'WHERE id = %s'
            for i in range(0, 10):
                if check == 0:
                    #실행한다 insert.geo, dinsert.Maxtem, dinsert.Mintem, dinsert.Humi, dinsert.Hpa, dinsert.Ozone는 %s에 들어갈 내용들이다.
                    curs.execute(sql1, (i+1, newsinsert[i][0], newsinsert[i][1], newsinsert[i][2], 1))
    ...
    try:
        ...
        with conn.cursor() as curs:
            ...
            for i in range(0, 16):
                if check == 0:
                #실행한다 insert.geo, dinsert.Maxtem, dinsert.Mintem, dinsert.Humi, dinsert.Hpa, dinsert.Ozone는 %s에 들어갈 내용들이다.
                    curs.execute(sql1, (i+1, geome[i], dinsert[i][0], dinsert[i][1], dinsert[i][2], dinsert[i][3], dinsert[i][4]))
                else :
                    curs.execute(sql2, (dinsert[i][0], dinsert[i][1], dinsert[i][2], dinsert[i][3], dinsert[i][4], i+1))
        #삽입 내용을 확정시킨다.
        #안하면 저장이 안된다.
        conn.commit()
    finally:
